...
describe('RangePicker', () => {
  ...
  it('click show time panel', () => {
    const picker = mount(<RangePicker defaultValue={[moment('2019-02-16 18:01:57'), moment('2019-02-20 18:01:57')]}/>);
    const node = picker.find('.RangePicker');
    node.simulate('click');
    picker.find('a.rc-calendar-time-picker-btn').at(0).simulate('click');
    expect(picker.find('.rc-calendar-time-picker').length).toBe(2);
  });
  it('clear choosed duration', () => {
    const picker = mount(<RangePicker defaultValue={[moment('2019-02-16 18:01:57'), moment('2019-02-20 18:01:57')]} onOk={onOk}/>);
    const node = picker.find('.RangePicker');
    node.simulate('mouseenter');
    picker.find('.icon-close').at(0).simulate('click');
  ...
  it('3 hours duration', () => {
    const picker = mount(<RangePicker onChange={onChange} onOk={onOk} onOpenChange={onOpenChange}/>);
    const node = picker.find('.RangePicker');
    node.simulate('click');
    picker.find('.RangePicker__duration button').at(0).simulate('click');
    const inputs = picker.find('input');
    // 开始时间
