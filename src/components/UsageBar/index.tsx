  ...
  return (
    <div className={`UsageBar ${inline ? 'inline' : ''}`}>
      <ProgressBar>
        ...
        // style用于展示最小进度，不满时1px以1px展示，考虑以百分比展示时的最小展示进度，只要perc >= 0.000015，都会展示为0.01%，应展示最小进度；但如果展示的是0.00%，实际上是有很小的值，但从体验层面上，应该不展示进度，故需要对临界点做处理
        <ProgressBar className={bsStyle && `progress-bar-${bsStyle}`} now={perc} max={1} key={index} 
          style={{'minWidth': (!hasPercent && !isPercent && perc > 0) ? '1px': isPercent && hasPercent && perc >= 0.000015 ? '1px' : '0px'}}/>))}
        </ProgressBar>
      {waterLine && (
