...
describe('VirtualSelectBox', () => {
  ...
  it('render with async datas', async () => {
    ...
    const node = picker.find('.SelectBox');
    // default value
    expect(picker.find('.SelectBox__btn > span').at(0).props().children).toBe('osUser-1');
    expect(picker.find('.icon-close').exists()).toBeTruthy();
    node.find('Glyphicon').simulate('click');
    await sleep(500);
    picker.update();
    // 默认第二个高亮
    expect(picker.find('.VirtualList > .SelectBox__item').at(1).hasClass('active')).toEqual(true);
    // 选择第三个
    picker.find('.VirtualList > .SelectBox__item').at(2).simulate('click');
    picker.update();
    // 展开自动隐藏
